...
class AuthControllerTest extends TestCase
{
    ...
    {
        // Regression guard: without Auth::guard('fan-session')->login()
        // in AuthController::sessionResponse(), this response's session
        // cookie is regenerated by Sanctum's stateful middleware but never
        // bound to the fan account, so login() reports success yet every
        // subsequent cookie-based request (GET /profile, ...) stays
        // unauthenticated in a real browser — the same gap already fixed
        // for AdminAuthController.
        UserModel::factory()->create(['email' => 'cookie-check@example.com', 'password_hash' => Hash::make('Senha123')]);

        $response = $this->withHeader('Origin', 'http://localhost:3000')
            ->postJson('/api/v1/auth/login', ['email' => 'cookie-check@example.com', 'password' => 'Senha123']);
    ...
    public function test_GIVEN_a_session_authenticated_fan_WHEN_logging_out_THEN_the_session_is_invalidated(): void
    {
        $user = UserModel::factory()->create();
        $this->actingAs($user, 'fan-session');

        $response = $this->postJson('/api/v1/auth/logout');

        $response->assertStatus(200);
        $this->assertGuest('fan-session');
    }
